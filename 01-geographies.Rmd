...
While Census blocks and tracts form the building blocks of all geographic levels at which Census data is reported, not all geographic levels perfectly “nest.” Nested geographies refer to the hierarchical “spine” of nation > region > division > state > county > tract > block, where each unit at a lower level is fully contained within a single unit of the next level up, and each higher-level unit is completely partitioned into lower-level units. Non-nesting geographies still rely on Census blocks and tracts, but their boundaries sometimes cross those of higher levels (e.g., metropolitan statistical areas cross state lines, places like New York City cross county lines, etc.). Although Census blocks, tracts, and certain geographies such as congressional districts are collaboratively delineated so that Census data can be accurately aggregated to key geographies, many other geographies are defined independently (e.g., ZIP Codes). Because these independently defined geographies cannot be directly aggregated from Census blocks and tracks, the Census Bureau produces “tabulation areas” (e.g., ZIP Code Tabulation Areas or ZCTAs) that approximate non-Census geographies using Census blocks and tracts. As a specific tabulation area becomes more widely used, the Census Bureau sometimes works with the managers of the approximated geography to better reflect the geography with Census blocks and tracts (e.g., the Census Bureau working with NYC DCP to redraw block and tract boundaries to better reflect Community Districts). 
...
In the early 1900s, Dr. Walter Laidlaw attempted to use New York State Assembly Districts to study neighborhood conditions in NYC. When the state redrew its district boundaries in 1905, he concluded that Assembly Districts were too unstable for long-term demographic analysis. Thus, in 1906 he published a paper proposing a system of permanent small areas with boundaries that would remain consistent between Censuses. He then persuaded the newly created Census Bureau to pilot the idea in the 1910 Census, producing tract tabulations for NYC as well as Baltimore, Boston, Chicago, Cleveland, Philadelphia, Pittsburgh, and St. Louis. The group expanded for the 1930 Census to include Berkeley, Columbus, Los Angeles, Nashville, Syracuse, and Yonkers. In 1931, the American Statistical Association formed a Committee on Census Enumeration Areas and appointed Howard Whipple Green as chair; Mr. Green spent the next twenty-five years encouraging cities nationwide to adopt and use the new tracts.

Institutionally, the Census Bureau standardized the tract delineation process in 1934. For the 1940 Census, it also began reporting block-level data for all cities with at least 50,000 residents. In cities where Census tracts already existed, blocks were nested within them; in cities without tracts, the Bureau created “block areas” (renamed “block numbering areas” or BNAs in 1960) to function as equivalent geographies to tracts. During the 1970s and 1980s, BNAs were delineated across much of the country, with the 1990 Census being the first to assign every part of the US to both a Census block and a tract/BNA. The 2000 Census then retired the BNA system entirely and converted all BNAs into Census tracts.

<hr style="border:2px solid blue">

## Nested Geographies

Taking full advantage of the nesting of these geographies, the Census Bureau labels units in each level with GEOIDs that build upon the GEOIDs of higher levels of geography so the full code string contains information beyond identification: 
...
**Note:** Notice how "Census Tract 65" became "006500" in the GEOID. When split out in a separate column and named, tracts will often have whole numbers like 65 and decimals like 206.01 (as a new tract was split from prior tract 206). When reconstructing the GEOID from the component FIPS codes, check the tract names and multiply all values by 100 if there are decimals:

```{r eval = FALSE}
data %>% 
  dplyr::mutate(
    GEOID = stringr::str_c(
      stringr::str_pad(state_fips, width = 2, side = "left", pad = "0"),
      stringr::str_pad(county_fips, width = 3, side = "left", pad = "0"),
      stringr::str_pad(round(tract_number * 100), width = 6, side = "left", pad = "0"),
      stringr::str_pad(block_number, width = 4, side = "left", pad = "0")
    )
  )
```

Below is a description of each geographic level in the hierarchical “spine”, how it is called using `tidycensus`, and how NYC is coded. 
...
> The "nation" is the broadest geographic unit used for the presentation of Census data, representing the United States as a single summary entity. It encompasses the fifty states and the District of Columbia, though U.S. Territories, including Puerto Rico, are not part of the Nation for most Census data products.
...
```{r echo = FALSE}
readr::read_csv("Data/state_codes.csv") %>% 
  dplyr::transmute(
    State,
...
In NYC, PUMAs are an approximation of Community Districts (although the Census Bureau worked with NYC DCP to create new [Community District Tabulation Areas (CDTAs)](https://www.nyc.gov/content/planning/pages/resources/datasets/community-district-tabulation)).
...
## Re-Aggregating Census Data

Sometimes the desired geographic level is not directly available as a Census product (e.g., NYC's new Community District Tabulation Areas or CDTAs) or it may be unique to a specific analysis (e.g., rezonings). As long as the target geographic level can be constructued using Census blocks and tracts, Census data can be re-aggregated to produce estimates at that custom level. 

The first step in re-aggregating Census data is to locate or create a crosswalk of existing Census geographies to the target geography. Most commonly this is done at the Census tract level, though Census blocks can be used when the custom geography splits tracts, and larger units can be used when the target geography is built from higher-level Census areas. For example, tract-level data can be re-aggregated to CDTAs using a [crosswalk](https://www.nyc.gov/content/planning/pages/resources/datasets/community-district-tabulation) provided by the NYC DCP:

<br>

```{r echo = FALSE}
tract_cdta_crosswalk <-
  readr::read_csv("Data/tract_cdta_crosswalk.csv") %>% 
  dplyr::transmute(
...
Then, join data for the existing Census geographies to the crosswalk, group by the target geography, and sum the values. When working with surveys, the margin of error is calculated by taking the square root of the sum of squared margins of error being aggregated: 

$$MOE_{total}=\sqrt{\sum{MOE_{i}^2}}$$

Returning to the CDTA example:
...
tract_data <- 
  tidycensus::get_acs(
    ...
    variables = c(
      ...
      poverty_numerator = "B17001_002",
      poverty_denominator = "B17001_001"
    )
  ) %>% 
  dplyr::select(-NAME)
...
```{r eval = FALSE}
tract_data %>% 
  dplyr::inner_join(tract_cdta_crosswalk, by = "GEOID") %>% 
  dplyr::summarize(
    .by = c("CDTACode", "CDTAName", "variable"),
  ...
  dplyr::transmute(
    CDTACode,
    CDTAName = str_remove_all(CDTAName, CDTACode),
    `Population (estimate)` = estimate_population,
    `Population (moe)` = moe_population,
    `Poverty Rate (estimate)` = estimate_poverty_numerator / estimate_poverty_denominator,
    `Poverty Rate (moe)` = dplyr::if_else(
      moe_poverty_numerator^2 - (`Poverty Rate (estimate)`^2 * moe_poverty_denominator^2) >= 0,
      sqrt(moe_poverty_numerator^2 - (`Poverty Rate (estimate)`^2 * moe_poverty_denominator^2)) / estimate_poverty_denominator,
      sqrt(moe_poverty_numerator^2 + (`Poverty Rate (estimate)`^2 * moe_poverty_denominator^2)) / estimate_poverty_denominator
    )
  ) %>% 
  dplyr::arrange(CDTACode)
```
...
tract_data %>% 
  ...
  dplyr::arrange(CDTACode) %>% 
  DT::datatable() %>% 
  DT::formatRound(c("Population (estimate)", "Population (moe)"), digits = 0) %>% 
  DT::formatPercentage(c("Poverty Rate (estimate)", "Poverty Rate (moe)"), digits = 1)
```

<br>

**Note:** This process works cleanly for counts, sums, and shares; counts and sums are obtained by adding values across all constituent parts, while shares are obtained by summing the numerator and denominator across parts and then computing the ratio from those totals. Medians and other quantiles are harder to re-aggregate without the underlying microdata. One option is to compute a weighted average of the constituent medians using an appropriate size weight (e.g., population or households), though this is not a true median. Another option is to use a binned distribution, sum counts within each bin across parts, and then locate the bin containing the middle observation. This identifies the median observation in the distribution, but if bins are wide, inconsistently defined, or open-ended (e.g., $2,500+), the value of the median observation within the bin is not easily estimated. Similarly, if many target geographies have their median observations in the same bin, the resulting estimated values can be too coarse for meaningful comparisons.  
